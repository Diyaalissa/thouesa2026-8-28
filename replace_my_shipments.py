...
# We need to find: {activeTab === 'MY_SHIPMENTS' && ( ... )}
# It starts around 2157 and ends where the next activeTab is, probably {activeTab === 'WALLET'
start_idx = content.find("{activeTab === 'MY_SHIPMENTS' && (")
if start_idx == -1:
    print("Could not find MY_SHIPMENTS block")
    exit(1)

end_idx = content.find("{activeTab === 'WALLET' && (", start_idx)
if end_idx == -1:
            ...
            <div className={`space-y-3 ${selectedShipment ? 'hidden lg:block' : 'block'}`}>
              {senderShipments.filter(s => {
                ...
                senderShipments.filter(s => {
                  ...
                  // Payment Tag Logic
                  // Note: In real app, derived from shipment data. Using mock mapping here.
